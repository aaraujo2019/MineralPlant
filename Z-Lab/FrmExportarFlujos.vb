...
Public Class FrmExportarFlujos
    ...
    Private Sub exportarsolidos()
        ...
        If CmbUbicacionflujo.Text = "ESPESADOR4" Then
            RstResumen = conn.Execute(" SELECT Fecha, Turno, ToneladasTurno, PromedioDensidad, PromedioGravedad FROM  dbo.Pb_FlowsEspesador4 where     (Fecha >= '" & CDate(DtFechainicio.Text) & "') AND (Fecha <= '" & CDate(DtFechaFinal.Text) & "')  ORDER BY Fecha ")
            RstResumenTenor = conn.Execute(" SELECT     Fecha, Turno, AuFinal_ppm FROM         dbo.Pb_TenorEspesador4 where     (Fecha >= '" & CDate(DtFechainicio.Text) & "') AND (Fecha <= '" & CDate(DtFechaFinal.Text) & "')  ORDER BY Fecha ")
        ...
        ElseIf CmbUbicacionflujo.Text = "ESPESADOR2" Then
            RstResumen = conn.Execute(" SELECT Fecha, Turno, ToneladasTurno, PromedioDensidad, PromedioGravedad FROM  Pb_FlowsEspesador2 where     (Fecha >= '" & CDate(DtFechainicio.Text) & "') AND (Fecha <= '" & CDate(DtFechaFinal.Text) & "')  ORDER BY Fecha ")
            RstResumenTenor = conn.Execute(" SELECT     Fecha, Turno, AuFinal_ppm FROM         dbo.Pb_TenorEspesador2 where     (Fecha >= '" & CDate(DtFechainicio.Text) & "') AND (Fecha <= '" & CDate(DtFechaFinal.Text) & "')  ORDER BY Fecha ")
    ...
    Private Sub FrmExportarFlujos_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
