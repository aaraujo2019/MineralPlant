...
" WHERE RfUserEvent.Usuario='" & LblUsuario.Text & "'  and (RfUserEvent.IdEvento  = 'ModificarMuestrasLab'  ) ", Cn)
        ...
        Else
            Try
                ...
                If editamuestra = True Then
                    ...
                    cmd.CommandText = "UPDATE  PB_Samples  SET   HoraInicial= @HoraInicial ,  HoraFinal= @HoraFinal , Fecha = @Fecha , Ubicacion = @Ubicacion  , Dup = @Dup , TipoMuestra = @TipoMuestra , Comments= @Comments  WHERE  Muestra = @Muestra  "
                Else
    ...
    Private Sub cargarOperacion()
        ...
        Try
            ...
            If rstoperacion.EOF = True Then
                ...
                TxtFundicion.Clear()
            Else
                Txtconsumo.Text = CStr((rstoperacion.Fields("ConsumoEnergia").Value))
                ...
                ' fundicion
                TxtFundicion.Text = CStr((rstoperacion.Fields("OroFundido").Value))
  ...
  MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Private Sub Llenar_DataGridViewDgSamplesDay()
        ...
        Dim da As New SqlDataAdapter
        Dim sql As String
        sql = " SELECT    Muestra, Fecha, HoraInicial,HoraFinal, Ubicacion,TipoMuestra, Dup , Comments  from PB_Samples WHERE Fecha=  '" & CDate(DateTimePickerFechaReporte.Text) & "'  ORDER BY TipoMuestra , Muestra"
        da.SelectCommand = New SqlCommand(sql, Cn)
        da.Fill(ds)
        Cn.Close()
        Dim dt As DataTable = ds.Tables(0)
    ...
    Private Sub Llenar_DataGridViewDgFlujometroE5()
        ...
        Dim da As New SqlDataAdapter
        Dim sql As String
        sql = "SELECT        TOP (100) PERCENT dbo.Pb_FlowsE5.LecturaInicial, dbo.Pb_FlowsE5.LecturaFinal , dbo.Pb_FlowsE5.Densidad,HorasOperacion ,dbo.pb_flowse5.gespecifica , dbo.pb_flowse5.FlujoM3 , dbo.pb_flowse5.TonsTurno , dbo.Pb_FlowsE5.Turno  FROM      dbo.Pb_FlowsE5  WHERE    (dbo.Pb_FlowsE5.Fecha = '" & CDate(DateTimePickerFechaReporte.Text) & "') ORDER BY dbo.Pb_FlowsE5.turno "
        da.SelectCommand = New SqlCommand(sql, Cn)
        da.Fill(ds)
        Cn.Close()
        Dim dt As DataTable = ds.Tables(0)
    ...
    Private Sub Llenar_DataGridViewDgFlujometroFlotacion()
        ...
        Dim da As New SqlDataAdapter
        Dim sql As String
        sql = "SELECT        TOP (100) PERCENT dbo.Pb_FlowsColasBulk.LecturaInicial, dbo.Pb_FlowsColasBulk.LecturaFinal , dbo.Pb_FlowsColasBulk.Densidad,HorasOperacion ,dbo.Pb_FlowsColasBulk.gespecifica , dbo.Pb_FlowsColasBulk.FlujoM3 , dbo.Pb_FlowsColasBulk.TonsTurno , dbo.Pb_FlowsColasBulk.Turno  FROM      dbo.Pb_FlowsColasBulk  WHERE    (dbo.Pb_FlowsColasBulk.Fecha = '" & CDate(DateTimePickerFechaReporte.Text) & "') ORDER BY dbo.Pb_FlowsColasBulk.turno "
        da.SelectCommand = New SqlCommand(sql, Cn)
        da.Fill(ds)
        Cn.Close()
        Dim dt As DataTable = ds.Tables(0)
...
ByVal e As EventArgs) Handles ChkTenorBanda.CheckedChanged
  ...
  MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Private Sub DgSamplesDay_CellClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DgSamplesDay.CellClick
        Try
            'Aquí pongo lo que pasaría si el campo está en blanco o nulo
  ...
  MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Private Sub DgSamplesDay_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DgSamplesDay.CellDoubleClick
...
" WHERE RfUserEvent.Usuario='" & LblUsuario.Text & "'  and (RfUserEvent.IdEvento  = 'ModificarMerrilCrowe'  ) ", Cn)
        ...
        Else
            Try
                ...
                cmd.Parameters.AddWithValue("@Fecha", CDate(DateTimePickerFechaReporte.Text))
                cmd.Parameters.AddWithValue("@LecturaInicial", Convert.ToDecimal(TxtInicioMC.Text))
                cmd.Parameters.AddWithValue("@LecturaFinal", Convert.ToDecimal(TxTFinalMC.Text))
                cmd.Connection = sqlConnectiondb
                sqlConnectiondb.Open()
                cmd.ExecuteNonQuery()
                sqlConnectiondb.Close()
...
" WHERE RfUserEvent.Usuario='" & LblUsuario.Text & "'  and (RfUserEvent.IdEvento  = 'ModificarMineralPlant'  ) ", Cn)
        ...
        Try
            ...
            If editaroperacion = True Then
                cmd.CommandText = "UPDATE PB_Operation SET  [ConsumoEnergia] = @ConsumoEnergia , [TonHora] = @TonHora , [OperacionHorasDia] =@OperacionHorasDia , [DetencionMtto] = @DetencionMtto, [DetencionOperacion]=@DetencionOperacion,  [TonHumedasZC]=@TonHumedasZC, [TonhumedasPM]=@TonhumedasPM, [StockZCGruesos]=@StockZCGruesos, [StockZCFinos]=@StockZCFinos, [StockPMFinos]=@StockPMFinos, [TonMolidasZandor]=@TonMolidasZandor , [TonMolidasMineras]=@TonMolidasMineras ,[OroFundido]=@OroFundido   WHERE [Fecha] =   '" & Format(fechaestimada, "yyyy/MM/dd") & "'   "
            Else
                cmd.CommandText = "INSERT INTO PB_Operation (Fecha,ConsumoEnergia,TonHora, OperacionHorasDia,  DetencionMtto, DetencionOperacion, TonHumedasZC, TonhumedasPM, StockZCGruesos, StockZCFinos, StockPMFinos, TonMolidasZandor ,TonMolidasMineras, OroFundido)VALUES(@Fecha,@ConsumoEnergia,@TonHora, @OperacionHorasDia,  @DetencionMtto, @DetencionOperacion, @TonHumedasZC, @TonhumedasPM, @StockZCGruesos, @StockZCFinos, @StockPMFinos , @TonMolidasZandor , @TonMolidasMineras, @OroFundido)"
            End If
            cmd.Parameters.AddWithValue("@Fecha", Convert.ToDateTime(DateTimePickerFechaReporte.Value))
            cmd.Parameters.AddWithValue("@ConsumoEnergia", Convert.ToString(Txtconsumo.Text))
            ...
            ' fundicion
            cmd.Parameters.AddWithValue("@OroFundido", Convert.ToString(TxtFundicion.Text))
            cmd.Connection = sqlConnectiondb
            sqlConnectiondb.Open()
            cmd.ExecuteNonQuery()
            sqlConnectiondb.Close()
...
" WHERE RfUserEvent.Usuario='" & LblUsuario.Text & "'  and (RfUserEvent.IdEvento  = 'ModificarHorometro'  ) ", Cn)
        ...
        If myDataViewpermisos.Count = 0 Then
            ...
            Exit Sub
        End If
        If TxtInicioE5.Text = "" Or TxtFinalE5.Text = "" Or TxtHorasOperacione5.Text = "" Then
            MsgBox("Todos los campos son obligatorios, por favor Diligencie correctamente el formulario")
        Else
            Try
                Dim sqlConnectiondb As New System.Data.SqlClient.SqlConnection(ConfigurationManager.AppSettings("StringConexion").ToString)
                ...
                If editarflujoe5 = True Then
                    'CONSULTA CUANDO EDITAR ES VERDADERO
                    cmd.CommandText = "UPDATE  Pb_FlowsE5  SET LecturaInicial = @LecturaInicial,  LecturaFinal= @lecturafinal , densidad = @densidad , HorasOperacion=@HorasOperacion  WHERE Turno=@Turno and Fecha=@Fecha  "
                Else
                    ' consulta sql si editar es falso 
                    cmd.CommandText = "INSERT INTO Pb_flowse5 (LecturaInicial,LecturaFinal, densidad , HorasOperacion ,Fecha, Turno)VALUES(@LecturaInicial, @LecturaFinal , @Densidad , @HorasOperacion ,@Fecha, @Turno)"
                End If
    ...
    Private Sub exportarColasFlotacion()
        ...
        recorrido = 2
        objExcel.Visible = False
        RstResumen = conn.Execute(" SELECT  Turno, FlujoM3, TonsTurno, HorasOperacion, Densidad, Fecha FROM  dbo.Pb_FlowsColasBulk where     (Fecha >= '" & CDate(DtInicioFl.Text) & "') AND (Fecha <= '" & CDate(DtFinalFl.Text) & "')  ORDER BY Fecha ")
        With objExcel
            'concentrado de flotacion
...
" WHERE RfUserEvent.Usuario='" & LblUsuario.Text & "'  and (RfUserEvent.IdEvento  = 'ModificarHorometro'  ) ", Cn)
        ...
        If myDataViewpermisos.Count = 0 Then
            ...
            Exit Sub
        End If
        If TxtInicioFl.Text = "" Or TxtFinalFl.Text = "" Or TxtHorasFl.Text = "" Or TxtDensidadFl.Text = "" Then
            MsgBox("Todos los campos son obligatorios, por favor Diligencie correctamente el formulario")
        Else
            Try
                Dim sqlConnectiondb As New System.Data.SqlClient.SqlConnection(ConfigurationManager.AppSettings("StringConexion").ToString)
                ...
                If editaflujoflotacion = True Then
                    'CONSULTA CUANDO EDITAR ES VERDADERO
                    cmd.CommandText = "UPDATE  Pb_FlowsColasBulk  SET LecturaInicial = @LecturaInicial,  LecturaFinal= @lecturafinal , densidad = @densidad , HorasOperacion=@HorasOperacion  WHERE Turno=@Turno and Fecha=@Fecha  "
                Else
                    ' consulta sql si editar es falso 
                    cmd.CommandText = "INSERT INTO Pb_FlowsColasBulk (LecturaInicial,LecturaFinal, densidad , HorasOperacion ,Fecha, Turno)VALUES(@LecturaInicial, @LecturaFinal , @Densidad , @HorasOperacion ,@Fecha, @Turno)"
                End If
    ...
    Private Sub exportarflujoE5()
        ...
        recorrido = 2
        objExcel.Visible = False
        RstResumen = conn.Execute(" SELECT  Turno, FlujoM3, TonsTurno, HorasOperacion, Densidad, Fecha FROM  dbo.Pb_FlowsE5 where     (Fecha >= '" & CDate(DtInicioE5.Text) & "') AND (Fecha <= '" & CDate(DtFinalE5.Text) & "')  ORDER BY Fecha ")
        With objExcel
            'concentrado de flotacion
            hoja = CType(.Sheets("Data"), Microsoft.Office.Interop.Excel.Worksheet)
            'hoja.Cells(1, 3) = RstResumen.Fields("Ubicacion").Value
