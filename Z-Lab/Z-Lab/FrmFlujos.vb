...
Public Class FrmFlujos
    ...
    Dim Cn As New SqlConnection("Server=10.10.203.4\gcg;uid=sa;pwd=BdZandor123*;database=PlantaBeneficio")
...
" WHERE RfUserEvent.Usuario='" & LblUsuario.Text & "'  and (RfUserEvent.IdEvento  = 'ModificarFlujos'  ) ", Cn)
        ...
        Else
            Try
                Dim sqlConnectiondb As New System.Data.SqlClient.SqlConnection("Server=10.10.203.4\gcg;uid=sa;pwd=BdZandor123*;database=PlantaBeneficio")
  ...
  MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Private Sub CmdExportar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmdExportar.Click
        ...
        Dim cnStr As String
        cnStr = "Provider=SQLNCLI10;Initial Catalog=PlantaBeneficio;Data Source=10.10.203.4; User ID=sa;Password=BdZandor123*;"
        conn.Open(cnStr)
        ...
        RstResumen = conn.Execute(" SELECT     Fecha, HoraInicio, Densidad, Ubicacion , M3Hora, TonHora, GravedadEspecifica , PorcSolidos  FROM         dbo.PB_Flows  INNER JOIN RfFlows ON PB_Flows.Ubicacion = RfFlows.Name  where    (ubicacion = '" & (cmbubicacion.Text) & "') AND  (Fecha >= '" & CDate(DtFecha1.Text) & "') AND (Fecha <= '" & CDate(DtFecha2.Text) & "')  ORDER BY Fecha, Ubicacion ")
